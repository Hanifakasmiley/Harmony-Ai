...
### Requirement 11: Key Performance Indicator (KPI) Cards

**User Story:** As a System Administrator, I want to see key metrics at a glance, so that I can quickly understand the system's health and status.

#### Acceptance Criteria

1. WHEN the System Administrator accesses the dashboard, THE system SHALL display KPI cards at the top showing critical metrics
2. THE KPI cards SHALL display: Total Users, Active Patients, Total Sessions, High-Risk Alerts, Average Mood Score, Counsellor Utilization Rate
3. EACH KPI card SHALL show a large number (the metric value) with a descriptive label below it
4. EACH KPI card SHALL include a trend indicator (↑ for increase, ↓ for decrease, → for stable) with percentage change from previous period
5. THE KPI cards SHALL be color-coded based on status (Green for healthy, Yellow for warning, Red for critical)
6. WHEN a KPI card is clicked, THE system SHALL navigate to the relevant data table or detailed view

### Requirement 12: Trend Chart - Mood Over Time

**User Story:** As a System Administrator, I want to see mood trends over the last 7 days, so that I can monitor patient mental health trends.

#### Acceptance Criteria

1. WHEN the System Administrator views the dashboard, THE system SHALL display a line chart showing average mood levels over the last 7 days
2. THE chart SHALL display daily average mood scores (1-10 scale) for each day
3. THE chart SHALL include a trend line showing the overall direction (improving, declining, stable)
4. THE chart SHALL be interactive with hover tooltips showing exact values for each day
5. THE chart SHALL include a legend identifying the mood line and trend line
6. IF insufficient data exists, THE system SHALL display a message "Insufficient data for trend analysis"

### Requirement 13: Recent Activity Feed

**User Story:** As a System Administrator, I want to see recent system activity, so that I can stay informed about important events.

#### Acceptance Criteria

1. WHEN the System Administrator views the dashboard, THE system SHALL display a "Recent Activity" section showing the last 5-10 events
2. THE activity feed SHALL include events such as: New user registration, New session scheduled, Crisis alert triggered, User mood significantly changed, Counsellor assigned to patient
3. EACH activity item SHALL display: Event type (with icon), Description, Timestamp (e.g., "5 minutes ago", "2 hours ago")
4. EACH activity item SHALL be color-coded by event type (Blue for registration, Green for session, Red for crisis, Orange for mood change)
5. THE activity feed SHALL be sortable by timestamp (newest first)
6. WHEN an activity item is clicked, THE system SHALL navigate to the relevant record or detailed view
7. THE activity feed SHALL auto-refresh every 30 seconds to show new events

### Requirement 14: Dashboard Summary Section

**User Story:** As a System Administrator, I want a quick summary of system status, so that I can understand the overall health at a glance.

#### Acceptance Criteria

1. WHEN the System Administrator views the dashboard, THE system SHALL display a summary section below the KPI cards
2. THE summary section SHALL include: System Status (Online/Offline), Last Data Refresh timestamp, Number of active sessions today, Number of new alerts today
3. THE summary section SHALL display a "Refresh Now" button to manually update all dashboard data
4. THE summary section SHALL show a loading indicator while data is being fetched
5. IF the system is offline or API is unavailable, THE system SHALL display a warning banner with status information
